...
local function lock_group_heavy(msg, data, target)
  ...
  local group_etehad_lock = data[tostring(target)]['settings']['heavey']
  if group_heavy_lock == 'yes' then
    return 'heavy setting is already locked'
  else
    data[tostring(target)]['settings']['heavy'] = 'yes'
    save_data(_config.moderation.data, data)
    return 'heavy setting has been locked'
  end
end
...
local function unlock_group_heavy(msg, data, target)
  ...
  if group_heavy_lock == 'no' then
    return 'heavy setting is not locked'
  else
    data[tostring(target)]['settings']['heavy'] = 'no'
    save_data(_config.moderation.data, data)
    return 'heavy setting has been unlocked'
  end
end
...
local function lock_group_easy(msg, data, target)
  ...
  if group_easy_lock == 'yes' then
    return 'easy setting is already locked'
  else
    data[tostring(target)]['settings']['easy'] = 'yes'
    save_data(_config.moderation.data, data)
    return 'easy setting has been locked'
  end
end
...
local function unlock_group_easy(msg, data, target)
  ...
  local group_easy_lock = data[tostring(target)]['settings']['easy']
  if group_easy_lock == 'no' then
    return 'easy setting is not locked'
  else
    data[tostring(target)]['settings']['easy'] = 'no'
    save_data(_config.moderation.data, data)
    return 'easy setting has been unlocked'
  end
end

local function lock_group_docment(msg, data, target)
  if not is_momod(msg) then
    return
  end
  local group_docment_lock = data[tostring(target)]['settings']['lock_docment']
  if group_docment_lock == 'yes' then
    return 'docment posting is already locked'
  else
    data[tostring(target)]['settings']['lock_docment'] = 'yes'
    save_data(_config.moderation.data, data)
    return 'docment posting has been locked'
  end
end

local function unlock_group_docment(msg, data, target)
  if not is_momod(msg) then
    return
  end
  local group_video_lock = data[tostring(target)]['settings']['lock_docment']
  if group_docment_lock == 'no' then
    return 'docment posting is not locked'
  else
    data[tostring(target)]['settings']['lock_docment'] = 'no'
    save_data(_config.moderation.data, data)
    return 'docment posting has been unlocked'
  end
end
...
local function lock_group_user(msg, data, target)
  ...
  if group_user_lock == 'yes' then
    return 'user posting is already locked'
  else
    data[tostring(target)]['settings']['lock_user'] = 'yes'
    save_data(_config.moderation.data, data)
    return 'user posting has been locked'
  end
end
...
local function unlock_group_user(msg, data, target)
  ...
  if group_user_lock == 'no' then
    return 'user posting is not locked'
  else
    data[tostring(target)]['settings']['lock_user'] = 'no'
    save_data(_config.moderation.data, data)
    return 'user posting has been unlocked'
  end
end
...
local function lock_group_join(msg, data, target)
  ...
  if group_join_lock == 'yes' then
    return 'group joining is already locked'
  else
    data[tostring(target)]['settings']['lock_join'] = 'yes'
    save_data(_config.moderation.data, data)
    return 'group joining has been locked'
  end
end
...
local function unlock_group_join(msg, data, target)
  ...
  if group_join_lock == 'no' then
    return 'join joining is not locked'
  else
    data[tostring(target)]['settings']['lock_join'] = 'no'
    save_data(_config.moderation.data, data)
    return 'join joining has been unlocked'
  end
end
...
local function lock_group_gif(msg, data, target)
  ...
  if group_gif_lock == 'yes' then
    return 'gif posting is already locked'
  else
...
local function unlock_group_gif(msg, data, target)
  ...
  if group_gif_lock == 'no' then
    return 'gif posting is not locked'
  else
...
local function lock_group_bots(msg, data, target)
  ...
  if group_bots_lock == 'yes' then
    return 'bots proctisn is enable'
  else
    data[tostring(target)]['settings']['lock_bots'] = 'yes'
    save_data(_config.moderation.data, data)
    return 'bots proctisn has been locked'
  end
end
...
local function unlock_group_bots(msg, data, target)
  ...
  if group_bots_lock == 'no' then
    return 'bots proctisn is not locked'
  else
    data[tostring(target)]['settings']['lock_bots'] = 'no'
    save_data(_config.moderation.data, data)
    return 'bots proctisn has been unlocked'
  end
end
...
local function unlock_group_audio(msg, data, target)
  ...
  local group_link_lock = data[tostring(target)]['settings']['lock_audio']
...
function show_supergroup_settingsmod(msg, target)
	...
	if data[tostring(target)]['settings'] then
		if not data[tostring(target)]['settings']['heavey'] then
			data[tostring(target)]['settings']['heavey'] = 'no'
		end
	end
	if data[tostring(target)]['settings'] then
	...
	if data[tostring(target)]['settings'] then
		if not data[tostring(target)]['settings']['lock_docment'] then
			data[tostring(target)]['settings']['lock_docment'] = 'no'
		end
	end
	if data[tostring(target)]['settings'] then
...
local function run(msg, matches)
	...
	local data = load_data(_config.moderation.data)
		...
		lock_group_docment(msg, data, target),
		...
		lock_group_audio(msg, data, target),
      	...
      	return lock_group_all(msg, data, target), all
      end
	   ----------------------
	   if matches[2] == 'etehad' then
		...
		unlock_group_docment(msg, data, target),
	  ...
	  if matches[2] == 'heavey' then
		...
		unlock_group_audio(msg, data, target),
      	...
      	return lock_group_heavey(msg, data, target), heavey
      end
	  -------------------------------
	  if matches[2] == 'easy' then
		...
		unlock_group_audio(msg, data, target),
      	...
      	return lock_group_easy(msg, data, target), easy
      end
	  -------------------------------
            ...
            if matches[2]:lower() == 'docment' then
				savelog(msg.to.id, name_log.." ["..msg.from.id.."] locked docment ")
				return lock_group_docment(msg, data, target)
			end 
		    ...
		    if matches[2]:lower() == 'share' then
				savelog(msg.to.id, name_log.." ["..msg.from.id.."] locked share contact posting ")
			...
			if matches[2]:lower() == 'join' then
				savelog(msg.to.id, name_log.." ["..msg.from.id.."] locked joining group ")
...
--started unlockes
    ...
    unlock_group_audio(msg, data, target)
        ...
        return unlock_group_all(msg, data, target), all
      end
     —------------------—
     if matches[2] == 'etehad' then
        local etehad ={
    ...
    unlock_group_audio(msg, data, target)
        ...
        return unlock_group_heavey(msg, data, target), heavey
      end
    —---------------------------—
    if matches[2] == 'easy' then
        local easy ={
    ...
    unlock_group_audio(msg, data, target)
        ...
        return unlock_group_easy(msg, data, target), easy
      end
    —---------------------------—
			...
			if matches[2]:lower() == 'docment' then
        savelog(msg.to.id, name_log.." ["..msg.from.id.."] locked docment ")
        return unlock_group_docment(msg, data, target)
      end 
        if matches[2]:lower() == 'video' then
        savelog(msg.to.id, name_log.." ["..msg.from.id.."] locked video posting ")
